...
public class RemoveDuplicate {
    public static void main(String[] args) {
        List<Staff> staffList1 = new ArrayList<>(Arrays.asList(
                new Staff("1", "nakamoto"),
                new Staff("2", "takagi"),
                new Staff("3", "katoh"),
                new Staff("4", "shimura")
        ));
        List<Staff> staffList2 = new ArrayList<>(Arrays.asList(
                new Staff("1", "AAA"),
                new Staff("2", "BBB")
        ));
        System.out.println(staffList1);
        System.out.println(staffList2);
        List<String> idList = staffList2.stream()
                .map(x -> x.getId())
                .collect(Collectors.toList());
        List<Staff> result = staffList1.stream()
