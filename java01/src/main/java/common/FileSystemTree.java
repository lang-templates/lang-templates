...
public class FileSystemTree extends JTree {
  ...
  public FileSystemTree(String dir) {
    ...
    final DefaultMutableTreeNode root =
        (dir != null) ? new DefaultMutableTreeNode(new File(dir)) : new DefaultMutableTreeNode();
    ...
    //                // collapseAll(tree); // StackOverflowError when collapsing nodes below 2nd
    // level
    //                if (isAdjusting.get()) {
    //                    return;
    //                }
  ...
  public void updateUI() {
    ...
    setCellRenderer(
        (tree, value, selected, expanded, leaf, row, hasFocus) -> {
          Component c =
              r.getTreeCellRendererComponent(tree, value, selected, expanded, leaf, row, hasFocus);
          if (selected) {
